...
#define LOG_NORMALS_STATISTICS 0
...
void NormalEstimationPcl<PointT>::computeNormals(PointCloudT& pcl, KdTreeT& kdtree, const pcl::PointXYZ& center)
{
    ...
    return computeNormalsByDirectionPropagation(pcl, kdtree, center);
    ...
    if (n > 0)
    {
        atom_bool_vec done(pcl.size(),false); 
        atom_bool_vec propagate(pcl.size(),false);         
...
template<typename PointT>
void NormalEstimationPcl<PointT>::computeNormalsByDirectionPropagation(PointCloudT& pcl, KdTreeT& kdtree, const pcl::PointXYZ& center)
{    
    ZTimer ztimer;
    const size_t n = pcl.size();
    if (n > 0)
    {
        ...
        for(size_t j=0, jEnd=pcl_laser_trajectory_.size(); j < jEnd; j++)
        {
            ...
            if(found==0)
            {
                found = kdtree.nearestKSearch(center_point, 1, pointIdxSearch, pointSquaredDistance);
            }
            if(found>0) 
            {
    ...
    {
        ROS_WARN("NormalEstimationPcl::computeNormalsByPropagation: received empty point cloud");
    }

    double tt = ztimer.measure();
    ROS_INFO("NormalEstimationPcl::computeNormalsByPropagation: computed %ld normals in %fs (%f normal/s)", n, tt, n / tt);   
}
