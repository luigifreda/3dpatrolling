...
const float TravAnalyzer::kRobotToAvoidRadius = TravAnalyzer::kRobotRadius; // 0.7 [m] other robot radius for computing the clearance (multi-robot traversability)  
...
//                }
            ...
            //if (wTot < 2) /// < do not use here a numerical threshold in this way!!!!
...
//    }
    ...
    if (b_enable_team_avoidance_ && b_set_team_future_trails_pcl_)
    {
        ...
        if(n_found > 0)
        {
            ...
            dist_future_trails_pcl = std::max(fast_sqrt(pointNKNSquaredDistance[0]) - kRobotToAvoidRadius, 0.f); 
            dist_squared = std::min(dist_squared, dist_future_trails_pcl*dist_future_trails_pcl);            
        }
    }
